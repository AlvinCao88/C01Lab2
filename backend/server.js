...
//Retrieve all the existing nodes for a specific user
app.get("/getAllNote", express.json(), async(req,res) => {
  try{
    // Verify the JWT from the request headers
    const token = req.headers.authorization.split(" ")[1];
    jwt.verify(token, "secret-key", async (err, decoded) => {
...
//Deleting an existing note given the id
...
//edit an existing node given the id
app.patch("/editNote/:noteId", express.json(), async (req, res) => {
  try {
    const { title, content } = req.body;
    
    // Check for a valid note ID
    const noteId = req.params.noteId;
    if (!ObjectId.isValid(noteId)) {
      return res.status(400).json({ error: "Invalid note ID." });
    }
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      // Find note with given ID and update it
      const collection = db.collection(COLLECTIONS.notes);
      const isUser = await collection.findOne({
        username: decoded.username,
        _id: new ObjectId(noteId),
